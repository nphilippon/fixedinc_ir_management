...
RUN R -q -e 'if (!requireNamespace("devtools", quietly=TRUE)) install.packages("devtools", repos="https://cloud.r-project.org")' && \
    ...
    R -q -e "install.packages(c('shiny', 'shinydashboard', 'quantmod', 'tidyquant', 'mgttr', 'DT', 'plotly', 'shinyjs', 'bslib', 'rstudioapi'), dependencies = TRUE, repos = 'https://packagemanager.rstudio.com/cran/latest')"
